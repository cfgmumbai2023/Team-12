...
const consumer = new mongoose.Schema({
    ...
    language: {
        type: String,
        required: [true, 'The language field is required!'],
        trim: true,
        maxlength: 100
    },
    board: {
...
consumer.pre('save', function (next) { // ? next means
    var consumer = this;
    if (consumer.isModified('password')) { //checking if password field is available and modified
...
consumer.statics.findByToken = function (token, callBack) {
    var consumer = this;
    jwt.verify(token, process.env.SECRETE, function (err, decode) { //this decode must give consumer._id if token is valid .ie decode=consumer_id
